...
```{r pressure, echo=TRUE}
  ...
  listProfit <- data.frame(qttAvExp = c(),qttProt = c(),qttAvExpNnProt = c(),qtteLog = c(),cout=c())
...

{
  ...
  listProfit <- rbind(listProfit,data.frame(qttAvExp = sum(vals),qttProt = sum(vals*listOut), qttAvExpNnProt = sum(vals*(1-listOut)),qtteLog = 0,cout = 0))
...
for (k in 1:nbIter){  
  ...
  listProfit <- rbind(listProfit,data.frame(qttAvExp = sum(vals),qttProt = sum(vals*listOut),qttAvExpNnProt = sum(vals*(1-listOut)),qtteLog = sum(s1@data@values*vals),cout = sum(s1@data@values*cost@data@values)))
